...
# 2
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'PAndRNB.csv')
P = []
R = []
counter = 0
...
plt.plot(P, R, color='green',
         label=r'NB (AUPR = %0.4f)' % (0.6677),linestyle='--',
         lw=2, alpha=1)

# 3
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'PAndRAdaboost.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.plot(P, R, color='purple',
         label=r'Adaboost (AUPR = %0.4f)' % (0.8771),linestyle='-.',
         lw=2, alpha=1)


# 4
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'PAndRLR.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.plot(P, R, color='orange',
         label=r'LR (AUPR = %0.4f)' % (0.7974),linestyle='-',
         lw=2, alpha=1)


# 5
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'PAndRXgboost.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.plot(P, R, color='deepskyblue',
         label=r'XGBoost (AUPR = %0.4f)' % (0.9094),linestyle=':',
         lw=2, alpha=1)

# 1
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'PAndRAttributeManner.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.plot(P, R, color='crimson',
         label=r'Proposed Method (AUPR = %0.4f)' % (0.9707),linestyle='-',
         lw=2, alpha=1)
...
plt.xlim([0, 1])
plt.ylim([0, 1])
plt.title('Comparison of Precision-Recall curve')
# 画网格
plt.grid(linestyle='-')
# 画对角线
plt.plot([1, 0], [0, 1], color='black', lw=2, linestyle='--')
plt.legend(fontsize='large')

plt.savefig('PR5fold.svg')
plt.savefig('PR5fold.tif')
