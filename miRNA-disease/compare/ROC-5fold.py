...
# 1
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'FprAndTprAttribute.csv')
P = []
R = []
counter = 0
...
plt.plot(P, R, color='green',
         label=r'Attribute (AUC=%0.4f)' % (roc_auc),linestyle=':', marker=',',
         lw=2, alpha=1)
# MyEnlarge(0, 0.7, 0.25, 0.25, 0.5, 0, 2, P, R, 2, color='black')

# 2
PAndRAttribute = []
ReadMyCsv(PAndRAttribute, 'FprAndTprManner.csv')
P = []
R = []
counter = 0
while counter < len(PAndRAttribute):
...
plt.plot(P, R, color='red',
         label=r'Embedding (AUC=%0.4f)' % (roc_auc),linestyle='-.',
         lw=2, alpha=1)
...
plt.plot(P, R, color='navy',
         label=r'Attribute+embedding (AUC=%0.4f)' % (roc_auc),linestyle='-',
         lw=2, alpha=1)
