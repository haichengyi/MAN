...
def MyAllNodeFeature():
    AllNodeAttributeNum = []
    ReadMyCsv(AllNodeAttributeNum, 'ThirdFeature\AllNodeAttributeNum.csv')

    # 生成AllNodeManner
    counterP = 0
    while counterP < 5:
        LineEmbeddingName = 'ThirdFeature\\' + 'tr' + str(counterP) + '.txt'
        ...
        while counter < len(AllNodeAttributeNum):
            pair = []
            counter1 = 0
            while counter1 < 64:  # 如果节点孤立，则Feature全为0，128维
                pair.append(0)
                counter1 = counter1 + 1
